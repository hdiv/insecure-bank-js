...
const statements = ['schema.sql', 'data.sql']
    ...
    .flatMap(sql => sql.split(";").map(sql => sql.trim()).filter(sql => sql.length > 0));
statements.reduce((p, sql) => p.then(() => sequelize.query(sql)), Promise.resolve())
...
Object.keys(helpers).forEach(name => hbs.handlebars.registerHelper(name, helpers[name]));
