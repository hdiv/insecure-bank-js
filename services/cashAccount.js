...
module.exports = {
    findCashAccountsByUsername: username => {
        const str = "select * from cashaccount  where username='" + username + "'";
    ...
    getFromAccountActualAmount: async (account, transaction) => {
        ...
        const result = await sequelize.query(sql, {type: QueryTypes.SELECT, transaction: transaction, replacements: [account]});
