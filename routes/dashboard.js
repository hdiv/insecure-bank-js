...
router.get('/', async (req, res) => {
    ...
    const creditAccounts = await findCreditAccountsByUsername(principal.username);
...
router.post('/userDetail/certificate', async (req, res) => {
    ...
    res.writeHead(200, [['Content-Type', 'application/octet-stream'], ['Content-Disposition', `attachment;Certificate_=${accounts[0].name}`]]);
    res.end(certificate);
});
...
router.post('/userDetail/maliciouscertificate', async (req, res) => {
    ...
    res.writeHead(200, [['Content-Type', 'application/octet-stream'], ['Content-Disposition', `attachment;MaliciousCertificate_=${accounts[0].name}`]]);
    res.end(certificate);
});
...
router.post('/userDetail/newcertificate', certificateUpload, (req, res) => {
    ...
    if (uploadChecksum === checksum) {
        ...
        res.end(`<p>File '${file.originalname}' not processed, only previously downloaded malicious file is allowed</p>`)
    }
});
